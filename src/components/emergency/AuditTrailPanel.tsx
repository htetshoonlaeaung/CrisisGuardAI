    ...
    <div id="audit-trail-panel" className="space-y-4">
      ...
      >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className={`text-lg md:text-xl font-extrabold tracking-tight ${isLight ? 'text-zinc-950' : 'text-white'}`}>
                Immutable Triage Audit Trail &amp; Decision Log
              </h2>
            </div>
            <p className={`text-xs mt-0.5 ${isLight ? 'text-zinc-600' : 'text-zinc-400'}`}>
              Cryptographically timestamped inference traces generated by the SWI-Prolog reasoning engine.
            </p>
          </div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search audit trail by recommended action or domain keyword..."
            value={filterQuery}
      ...
      {loading ? (
        ...
        >
          Loading immutable triage records from database...
        </div>
        ...
        >
          No triage audit evaluations recorded yet. Run an emergency evaluation to generate logs.
        </div>
      ) : (
          ...
          {filteredAudits.map((item) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <h3 className={`font-extrabold text-base md:text-lg ${isLight ? 'text-zinc-950' : 'text-white'}`}>
                    {humanizeAction(item.recommended_action)}
                  </h3>
                </div>
              ...
              {item.facts_snapshot && item.facts_snapshot.length > 0 && (
                ...
                >
                  <span className={`text-[11px] font-mono mr-1 ${isLight ? 'text-zinc-500' : 'text-zinc-400'}`}>Facts Snapshot:</span>
              ...
              {item.prohibited_actions && item.prohibited_actions.length > 0 && (
                <div className={`text-xs p-2.5 rounded-lg flex items-start gap-2 border ${
                  ...
                  <span>
                    <strong>Strict Prohibition:</strong> {item.prohibited_actions[0]}
                  </span>
                </div>
              )}
            </div>
